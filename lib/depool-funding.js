...
export default (superclass) => class extends superclass {
    ...
    async sendTicktock() {
        const depoolAddress = await this.datastore.getFundingAddress();
    ...
    async sendStakeImplDecorator(electionId, maxFactor, retryAttempts) {
        ...
        const depoolAddress = await this.datastore.getFundingAddress();
        const { elections_start_before, elections_end_before } = await super.getConfigParam(15);
...
async function lookForDePoolEvent(client, depoolAddress, timeWindow, isWhatWeAreLookingFor) {
    const { result } = await client.net.query_collection({
        collection: 'messages',
        filter: {
            src: { eq: depoolAddress },
            msg_type: { eq: 2 },
            created_at: { gt: Math.floor(Date.now() / 1000) - timeWindow }
        },
        result: 'body'
    });
