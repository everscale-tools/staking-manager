...
class StakingManagementPolicy {
    constructor(client, datastore, everscaleConfigParams, config) {
        ...
        this.flags = {
            sendMessagesDirectly: _.chain(config).get('submitTransactionsWith', 'console').eq('console').value(),
            stakeSendingIsInProgress: false,
            postElectionsTicktockIsSent: false
        }

        this.getConfigMemoized = mem(_.bindKey(this.everscaleConfigParams, 'get'));
    }

    getConfig(id) {
        const freshOnly = _.some([34, 36], p => p === id);

        return freshOnly ? this.everscaleConfigParams.get(id) : this.getConfigMemoized(id);
    }
    ...
    submitTransaction(input, retryAttempts = 5) {
        ...
        const task = cb => {
            ...
            if (this.flags.sendMessagesDirectly) {
                this.sendMessage(message_encode_params)
