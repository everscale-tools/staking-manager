...
export default class StakingManagementPolicy {
    ...
    async sendStake(maxFactor = 3, retryAttempts = 5) {
        ...
        const lastElectionId = _
            .chain(await this.getPastElectionIds())
            .last()
            .defaultTo(activeElectionId)
            .value();
        const dbEntry = await this.datastore.getElectionsInfo(lastElectionId);
        ...
        if (publicKey) {
            ...
            if (stake > 0) {
                ...
                await this.callWebhook({
                    ...
                    context: `https://ton.live/validators/validatorDetails?publicKey=${publicKey}`
                });
    ...
    async callWebhook(json) {
        ...
        const opts = _.get(settings, 'webhooks.options');
